...
execute(Request = #request{rid = Rid,
    ...
    {ok, Data} = file:read_file("/home/renato/Downloads/desenvolvimento/cpd/git/certificado/lote_certificado.zip"),
    io:format("Data >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ~p~n~n",[Data]),
    {ok, FileUnziped} = zip:unzip(Data),
    io:format("FileUnziped >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ~p~n~n",[FileUnziped]),
    %"/home/renato/Downloads/desenvolvimento/cpd/git/certificado/test_signed/original.xml"
    FileXml = read_file_xml(FileUnziped),
    {Docs, _} = xmerl_scan:string(FileXml, [{namespace_conformant, true}]),
    %{Key, CertBin} = sign_256_key(),
    Key = read_private_key("/home/renato/Downloads/desenvolvimento/cpd/git/certificado/certificado_fabiano/private_key.pem"),
    CertBin = read_certificate("/home/renato/Downloads/desenvolvimento/cpd/git/certificado/certificado_fabiano/publicCert.pem"),
    SignedXml = sign(Docs, Key, rsa_sha256, CertBin),
    XmlFormater = transform_tuple_in_xml(SignedXml),
    create_file_signed("/home/renato/Downloads/desenvolvimento/cpd/git/certificado/test_signed/signed_xml.xml",XmlFormater),
    %Doc = strip(SignedXml), 
    ...
    Result = esaml_util:build_nsinfo(Ns, #xmlElement{
        ...
        content = [
            ...
            #xmlElement{name = 'ds:SignatureMethod',
                attributes = [#xmlAttribute{name = 'Algorithm', value = DigestValue}]},
            #xmlElement{name = 'ds:Reference',
                attributes = [#xmlAttribute{name = 'URI', value = ""}],
                content = [
