...
def init_oauth(app):
    ...
    # Prepare client_kwargs with SSL verification setting
    client_kwargs = {
        'scope': app.config['OAUTH2_SCOPE'],
        'token_endpoint_auth_method': 'client_secret_basic',  # Use Basic Auth
        'token_placement': 'header',  # Send token in Authorization header
    }
    
    # Disable SSL verification if configured (for development with self-signed certs)
    # Authlib uses 'requests' library by default, which accepts verify=False
